    ...
    <div className={`${colourSchemes[blok?.Colour_Scheme]?.background || "bg-gray-900"} py-24 sm:py-32`} {...storyblokEditable(blok)}>
      <Constrained_Width>
        <div className="mx-auto max-w-2xl lg:text-center">
          <p className={`text-base font-semibold leading-7 text-indigo-400`}>{blok?.Title}</p>
          <h2 className={`mt-2 text-3xl font-bold tracking-tight sm:text-4xl ${colourSchemes[blok?.Colour_Scheme]?.subtitle || "text-white"}`}>
            {blok?.Subtitle}
          </h2>
